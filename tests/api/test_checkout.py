...
def test_checkout_shipping_address_update_by_id(
    ...
    variables = {"checkoutId": checkout_id, "shippingAddressId": address_id}
    ...
    data = content["data"]["checkoutShippingAddressUpdate"]
    assert not data["errors"]
    assert address.pk == int(
        graphene.Node.from_global_id(data["checkout"]["shippingAddress"]["id"])[1]
    )
    checkout.refresh_from_db()
    assert checkout.shipping_address is not None
...
def test_checkout_billing_address_update_by_id(
    ...
    assert not data["errors"]
    assert address.pk == int(
        graphene.Node.from_global_id(data["checkout"]["billingAddress"]["id"])[1]
    )
    checkout.refresh_from_db()
    assert checkout.billing_address is not None
...
def test_checkout_billling_address_update_invalid_argument(
    ...
    assert len(data["errors"]) == 1
    assert data["errors"][0]["message"] == (
        "One and only one billing address or billing address ID " "should be provided."
    )
