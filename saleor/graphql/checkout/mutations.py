...
class CheckoutShippingAddressUpdate(BaseMutation, I18nMixin):
    ...
    class Arguments:
        ...
        shipping_address_id = graphene.ID(
            required=False,
            description=(
                "ID of the mailing address " "to where the checkout will be shipped."
            ),
        )
    ...
    def perform_mutation(
        cls, _root, info, checkout_id, shipping_address=None, shipping_address_id=None
    ):
        checkout = cls.get_node_or_error(
            info, checkout_id, only_type=Checkout, field="checkout_id"
        )
...
class CheckoutBillingAddressUpdate(CheckoutShippingAddressUpdate):
    ...
    class Arguments:
        ...
        billing_address_id = graphene.ID(
            required=False, description=("ID of the billing address of the checkout")
        )
    ...
    @classmethod
    def perform_mutation(
        cls, _root, info, checkout_id, billing_address=None, billing_address_id=None
    ):
        checkout = cls.get_node_or_error(
            info, checkout_id, only_type=Checkout, field="checkout_id"
        )
        ...
        if billing_address is not None:
            ...
            billing_address = cls.get_node_or_error(
                info, billing_address_id, only_type=Address, field="billing_address_id"
            )
